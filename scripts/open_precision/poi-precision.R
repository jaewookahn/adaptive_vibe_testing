...
# all
poi_per_corr <- sqldf("select * from dpoi, dopp where dpoi.sys = dopp.sys and dpoi.userno = dopp.userno "); cor(poi_per_corr$count, poi_per_corr$open_prec); plot(poi_per_corr$count, poi_per_corr$open_prec); abline(lm(poi_per_corr$open_prec~poi_per_corr$count))
